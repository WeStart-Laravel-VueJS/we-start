...
onMounted(() => {
    ...
    axios.get('/service/'+route.params.slug)
    .then(res => {
        service.value = res.data.data
        main_img.value = res.data.data.image
    })

})

const changeMain = (path) => {
    main_img.value = path
}
...
<template>
    ...
    <div class="container grid grid-cols-2 gap-6">
        <div>
            <img :src="main_img" alt="product" class="w-full main-img">
            <div class="grid grid-cols-5 gap-4 mt-4">
                <img @click="changeMain(service.image)" :src="service.image" alt="product2"
                    class="w-full cursor-pointer border slide-img"
                    :class="{'border-primary' : main_img == service.image}"
                    >
                <img @click="changeMain(img.path)" v-for="(img, i) in service.gallery" :key="i" :src="img.path" alt="product2" class="w-full cursor-pointer border slide-img"
                :class="{'border-primary' : main_img == img.path}"
                >
        ...
        <div>
            <h2 class="text-3xl font-medium uppercase mb-2">{{ service.name_trans }}</h2>
